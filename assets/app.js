(()=>{const cfg=window.GMAX_CONFIG||{};let products=[],client=null,floatTimer=null;const $=(s,p=document)=>p.querySelector(s),$$=(s,p=document)=>[...p.querySelectorAll(s)];const fmt=n=>'₦'+Number(n||0).toLocaleString('en-NG');const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));const configured=()=>!!(cfg.SUPABASE_URL&&cfg.SUPABASE_ANON_KEY);
...
function bind(){$$('[data-add]').forEach(b=>b.onclick=()=>add(b.dataset.add));$$('[data-view]').forEach(b=>b.onclick=()=>view(b.dataset.view))}
function add(id){const p=products.find(x=>String(x.id)===String(id));if(!p||p.in_stock===false)return toast('This item is out of stock.');const c=cart(),i=c.find(x=>String(x.id)===String(id));i?i.qty++:c.push({id:p.id,name:p.name,price:Number(p.price),category:p.category,image_url:p.image_url||'',qty:1});save(c);toast('Added to cart')}
function renderFeatured(){const e=$('#featured-products');if(!e)return;if(!products.length)return renderEmpty(e,'No products have been added yet.');let list=products.filter(p=>p.featured&&p.in_stock!==false).slice(0,8);if(!list.length)list=products.filter(p=>p.in_stock!==false).slice(0,8);e.innerHTML=list.map(card).join('');bind()}
...
function startFloatingRecommendations(){
  ...
  document.body.insertAdjacentHTML('beforeend','<aside class="gmax-suggestion" id="gmax-suggestion"><button class="suggest-close" id="suggest-close" aria-label="Close">×</button><div id="suggest-content"></div></aside>');
  const box=$('#gmax-suggestion'),content=$('#suggest-content');let last=-1;
  function rotate(){
    ...
    last=idx;const p=eligible[idx];
    content.innerHTML=`<div class="suggest-label">You may also like</div><div class="suggest-row"><div class="suggest-img">${art(p)}</div><div class="suggest-info"><strong>${esc(p.name)}</strong><span>${fmt(p.price)}</span><div class="suggest-actions"><button data-suggest-view="${esc(p.id)}">View</button><button class="buy" data-suggest-add="${esc(p.id)}">Add to cart</button></div></div></div>`;
    $('[data-suggest-view]',box).onclick=()=>view(p.id);
    $('[data-suggest-add]',box).onclick=()=>add(p.id);
  }
  rotate();floatTimer=setInterval(rotate,9000);
  $('#suggest-close').onclick=()=>{clearInterval(floatTimer);box.remove()};
}
function renderCart(){const list=$('#cart-list');if(!list)return;const c=cart();list.innerHTML=c.length?c.map(i=>`<div class="cart-item"><div class="cart-thumb">${i.image_url?`<img src="${esc(i.image_url)}">`:'🔌'}</div><div><strong>${esc(i.name)}</strong><div style="color:var(--blue);font-weight:900;margin-top:4px">${fmt(i.price)}</div><div class="qty"><button data-dec="${esc(i.id)}">−</button><b>${i.qty}</b><button data-inc="${esc(i.id)}">+</button><span class="remove" data-remove="${esc(i.id)}">Remove</span></div></div><strong>${fmt(i.price*i.qty)}</strong></div>`).join(''):'<div class="empty"><h3>Your cart is empty</h3><p>Add products from the store to continue.</p></div>';const total=c.reduce((s,i)=>s+i.price*i.qty,0);if($('#cart-subtotal'))$('#cart-subtotal').textContent=fmt(total);if($('#cart-total'))$('#cart-total').textContent=fmt(total);$$('[data-inc]').forEach(b=>b.onclick=()=>qty(b.dataset.inc,1));$$('[data-dec]').forEach(b=>b.onclick=()=>qty(b.dataset.dec,-1));$$('[data-remove]').forEach(b=>b.onclick=()=>{save(cart().filter(i=>String(i.id)!==String(b.dataset.remove)));renderCart()})}
function qty(id,d){let c=cart(),i=c.find(x=>String(x.id)===String(id));if(i){i.qty+=d;if(i.qty<=0)c=c.filter(x=>String(x.id)!==String(id));save(c);renderCart()}}
if($('#cart-open'))$('#cart-open').onclick=()=>{renderCart();$('#cart-modal').classList.add('show')};
